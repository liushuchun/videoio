...
int DoFrameExport(char*  pInputURL, float fSeekPointPercent, int  iDstWidth, int iDstHeight, enum AVPixelFormat  eDstPixelFmt, int iFrameCount, 
	serialize_params*& ppsFramesOutput, int iMaxFrameOutputSize, char* pExtra, int iInterval)
{
	...
	do
	{
		...
		ppsFramesOutput->width = iDstWidth;
		ppsFramesOutput->height = iDstHeight;
		ppsFramesOutput->channels = 3;
		std::cout << "Decode Info:: Height:" << ppsFramesOutput->height << "Width: " << ppsFramesOutput->width << "channels: " << ppsFramesOutput->channels << std::endl;

		for (iIndex = 0; iIndex < iCount; iIndex++)
		{
			iGotFrameCount = 0;

			if (ppsFramesOutput == NULL)
			{
				break;
			}
			...
			while (iGotFrameCount < iFrameCount && iGotFrameCount < MAX_FRAME_COUNT)
			{
				...
				if (pkt.stream_index == iVideoIndex && iRet == 0)
				{
					...
					if (iRet == 0)
					{
						...
						{
							iRet = ConvVideoPacket(pAVFrame, iDstWidth, iDstHeight, eDstPixelFmt, pSws_ctx, ppsFramesOutput->pContent, iGotFrameCount);
							if (iRet == 0)
							{
								iGotFrameCount++;
							}
			...
			ppsFramesOutput->count = iGotFrameCount;
		}


		ppsFramesOutput->length = sizeof(serialize_params) + sizeof(char)*iDstHeight*iDstWidth*3*iGotFrameCount;
		std::cout << "Decode Info:: Lenth:" << ppsFramesOutput->length << std::endl;
...
int ConvVideoPacket(AVFrame*  pAVFrame, int iTargetWidth, int iTargetHeight, enum AVPixelFormat  eDstPixelFmt, 
					struct SwsContext*  pSws_ctx, char*& psFramesOutput, int iFrameIndex)
{
	...
	//Use BGR24 only
	memcpy(psFramesOutput + iTargetWidth*iTargetHeight*3*iFrameIndex, dst_data[0], dst_linesize[0] * iTargetHeight);
	memcpy(psFramesOutput + iTargetWidth*iTargetHeight*(3*iFrameIndex+1), dst_data[1], dst_linesize[1] * iTargetHeight);
	memcpy(psFramesOutput + iTargetWidth*iTargetHeight*(3*iFrameIndex+2), dst_data[2], dst_linesize[2] * iTargetHeight);
